...
# Lazy init client only in LIVE mode
client = None
if not PAPER:
...
def _place_oco_after_fill(symbol, orderId, qty, tpPrice, slStop, slLimit, waitSec=60):
    """
    After the LIMIT_MAKER BUY is FILLED, place OCO (TP + SL).
    """
    # PAPER path
    ...
    import binance.enums as be  # explicit import
...
def place_order(o: OrderIn):
    ...
    try:
        res = client.create_order(
            symbol=o.symbol,
            side=be.SIDE_BUY,
            type=be.ORDER_TYPE_LIMIT_MAKER,
